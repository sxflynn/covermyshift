<template>
  <v-card flat>
    <v-card-title class="d-flex align-center pe-2">
      ...
      <v-spacer></v-spacer>List of Requests
      (TODO: Headers issue, specific column display, font and style change of data)
      ...
      <template v-slot:item.approved="{ item }">
        <v-dialog width="500">
          <template v-slot:activator="{ props }">
            <v-btn color="orange" v-bind="props" text="Accept / Decline"> </v-btn>
          </template>

          <template v-slot:default="{ isActive }">
            <v-card title="Request off">
              <v-chip size="compact" id="vchipDate"> Rachelle R. 12-02-2023 (Monday) (Vacation)</v-chip>
              <v-card-text>
              ...
              <v-card-actions>
                <div class="text-end">
                  <v-btn variant="tonal" color="green" rounded="xl" text="Accept" class="text-uppercase" label
                    size="x-large" id="acceptButton"></v-btn>
                    <v-dialog width="500">
            <template v-slot:activator="{ props }">
              <v-btn v-bind="props" variant="tonal" color="red" rounded="xl" text="Decline" class="text-uppercase" label
                    size="x-large" id="declineButton"></v-btn>
                       ...
                       <v-card title="Reasoning for Decline">
                        ...
                        <v-card-actions>
                        <v-btn variant="tonal" color="red" rounded="xl" text="Send & Decline" class="text-uppercase" label
                    size="x-large" id="finalDecline"></v-btn>
                        <v-spacer></v-spacer>

                        <v-btn
                          text="Cancel"
                          @click="isActive.value = false" id="declineCancel">               
                        </v-btn>
                        </v-card-actions>
                        </v-card>
                        </template>
                ...
                <v-btn text="Cancel" @click="isActive.value = false" id="adCancel"></v-btn>
